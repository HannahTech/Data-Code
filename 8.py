...
def get_group_id(group_name):
    ...
    # 2. Model Properties
    if key in ["modelproperties", "adskmodelproperties"]:
        try:
            return GroupTypeId.AdskModelProperties
        except:
            pass

    # 3. Standard UI Groups
    if key == "general":
        try:
    ...
    if key == "graphics":
        try:
            ...
            pass
    if key in ["materialsfinishes", "materialsandfinishes", "materials"]:
        try:
    ...
    # 4. Legacy BuiltInParameterGroup Fallback
    legacy_map = {
        "identitydata": BuiltInParameterGroup.PG_IDENTITY_DATA,
        "identity": BuiltInParameterGroup.PG_IDENTITY_DATA,
        "td_asset_identification": BuiltInParameterGroup.PG_IDENTITY_DATA,
        "modelproperties": BuiltInParameterGroup.PG_ADSK_MODEL_PROPERTIES,
        "adskmodelproperties": BuiltInParameterGroup.PG_ADSK_MODEL_PROPERTIES,
        ...
        "materialsfinishes": BuiltInParameterGroup.PG_MATERIALS,
        "materialsandfinishes": BuiltInParameterGroup.PG_MATERIALS,
        "materials": BuiltInParameterGroup.PG_MATERIALS,
    }
    
    legacy_group = legacy_map.get(key, None)
    if legacy_group is not None:
        try:
            return ParameterUtils.GetParameterGroupTypeId(legacy_group)
        except:
            return legacy_group
