...
# IN[0] receives Excel data matrix from Data.ImportExcel
excel_data = IN[0] if IN[0] else []
...
if not run_script or not excel_data:
    results.append("CREATION SKIPPED: Excel data invalid or Boolean set to False.")
else:
    # Get or create Shared Parameter File
    sp_file = app.OpenSharedParameterFile()
    
    if not sp_file:
        # Default path in Temp directory if no shared param file is active
        auto_txt_path = os.path.join(os.environ.get("TEMP", "C:\\Temp"), "Auto_SharedParameters.txt")
    ...
    if not sp_file:
        ...
        try:
            ...
            data_rows = excel_data[1:] if len(excel_data) > 1 else excel_data
            ...
            results.append(f"FAILED: Excel processing error - {str(e)}")
